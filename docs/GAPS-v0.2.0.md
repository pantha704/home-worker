...
The reviewed browser-local architecture is documented in `docs/local-first-production-architecture.md`. Its current slice supports ordered multi-page text-layer PDF extraction (up to 100 pages) plus PNG/JPEG and one-page scanned-PDF OCR in a dedicated worker, per-page progress, request-scoped replies, explicit cancellation during worker processing followed by a non-cancellable finalization state at the persistence boundary, PDF/PNG/JPEG signature and source-size validation before local persistence, bounded worker execution, OPFS content-addressed objects, IndexedDB immutable revisions, expected-revision conflict checks, Web Locks, quota/persistence gates, multipage A4 generation without the former silent line-cap truncation, and digest-verified `.homeworker` export/import. A real Chrome and Firefox acceptance test proves ordered multi-page extraction, PNG OCR, and reopen/edit/export with no `/v1/` document traffic. Page-level processing checkpoints resume after cancel/crash before metadata commit. Browser OCR now fills empty PDF pages (capped at 10 scanned pages) and the local review workspace shows the source beside extracted text. Encrypted/JavaScript/Launch/EmbeddedFile PDFs are rejected after parsing, not by a raw keyword scan of the first 1 MiB; visible text that only mentions those words is accepted. Browser OCR rejects oversized PNG/JPEG pixel counts. Unreferenced OPFS objects are swept after a successful local create under a shared persist lock, with in-flight object digests protected. Worker progress carries page text, including blank pages, and checkpoints bind extraction version with a 24h TTL. Parser isolation on Render, source formatting fidelity, full renderer parity, Safari/iOS acceptance, hosted live drills, and public-traffic hardening remain open gates.
